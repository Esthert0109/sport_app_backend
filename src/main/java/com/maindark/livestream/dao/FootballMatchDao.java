...
public interface FootballMatchDao extends BasicDao<FootballMatch> {
    ...
    @Select("select * from football_match where home_team_id=#{homeTeamId} and away_team_id=#{awayTeamId} and match_time >=#{nowSeconds} order by match_time asc limit 1")
    FootballMatch getFootballMatchByHomeTeamIdAndAwayTeamId(@Param("homeTeamId") Integer homeTeamId,@Param("awayTeamId")Integer awayTeamId,@Param("nowSeconds")Long nowSeconds);
    ...
    @Select("select t1.id,t1.competition_id,t1.home_team_id,t1.away_team_id," +
            ...
            "live_stream_collection co on t1.id= co.match_id where co.user_id=#{userId} order by t1.match_time desc limit 3")
