...
@Mapper
public interface BasketballLineUpDao {
    @Insert("insert into basketball_line_up(match_id, type,player_id, player_name,minutes, point, assists, steals, total_rebounds, " +
    ...
    void insertData(BasketballLineUp basketballLineUp);


    @Update("update basketball_line_up set minutes=#{minutes},point=#{point},assists=#{assists},steals=#{steals}," +
            ...
            "field_goals_attempts=#{fieldGoalsAttempts},field_goals_made=#{fieldGoalsMade}")
    void updateData(BasketballLineUp basketballLineUp);

    @Select("select count(1) from basketball_line_up where player_id=#{playerId} and match_id=#{matchId}")
    int queryExist(@Param("playerId")Long playerId,@Param("matchId")Long matchId);
    @Select("select * from basketball_line_up where type=#{type} and match_id=#{matchId}")
